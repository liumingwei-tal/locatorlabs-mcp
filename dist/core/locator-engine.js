...
class LocatorEngine {
    ...
    async getLocators(url, elementDescription) {
    ...
    async analyzePage(url, elementTypes) {
        ...
        if (elementTypes && elementTypes.length > 0) {
            filtered = allElements.filter((el) => elementTypes.some((t) => el.tagName.toLowerCase().includes(t) || el.type?.includes(t)));
        }
        const results = filtered.map((el) => {
    ...
    async generatePageObject(url, className, language) {
        const page = await this.browserManager.navigateTo(url);
    ...
    async validateLocator(url, locatorStr) {
        const page = await this.browserManager.navigateTo(url);
        try {
    ...
    generateTypeScriptPOM(className, elements, url) {
...
`;
    ...
    generateJavaScriptPOM(className, elements, url) {
        const inits = elements.map((e) => `    this.${e.name} = page.${e.locator};`).join("\n");
        return `/**
 * Page Object Model for: ${url}
 * Generated by LocatorLabs MCP
...
`;
    ...
    generatePythonPOM(className, elements, url) {
